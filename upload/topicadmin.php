...
/*
	...
	$Id: topicadmin.php 13492 2008-04-18 08:21:29Z liuqiang $
*/
...
if(in_array($action, array('moderate', 'delete', 'move', 'highlight', 'type', 'close', 'stick', 'digest', 'bump', 'recommend'))) {
	...
	if(!submitcheck('warnsubmit')) {
		...
		if(substr($status, -2, 1) && $warned) {
			showmessage('thread_warning_existence', NULL, 'HALTED');
		}

		checkreasonpm();

		$db->query("UPDATE {$tablepre}posts SET status='".bindec($warned.substr($status, -1, 1))."' WHERE pid='$pid'", 'UNBUFFERED');
		if($warned) {
