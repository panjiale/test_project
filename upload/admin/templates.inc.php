	...
	if(!submitcheck('editsubmit') && $delete != 'yes' && $reset != 'yes') {
		...
		if($template['templateid'] != 1) {
			...
			if(file_exists($defaulttpl) && md5_file($defaulttpl) != md5_file($filename)) {
				$resetbutton = ' <input  style="vertical-align: middle" type="button" class="btn" value="'.$lang['tpl_reset'].'" accesskey="r" onclick="location.href=\'admincp.php?action=templates&operation=edit&templateid='.$template['templateid'].'&fn='.$fn.'&keyword='.$keywordenc.'&reset=yes\'"> '.
					 (strtolower(fileext($fn)) == 'htm' ? '<input  style="vertical-align: middle" type="button" class="btn" value="'.$lang['tpl_diff'].'" onclick="location.href=\'admincp.php?action=templates&operation=edit&templateid='.$template['templateid'].'&fn='.$fn.'&keyword='.$keywordenc.'&checktpl=yes\'"> ' : '');
			}
			...
			if($checktpl && strtolower(fileext($fn)) == 'htm') {
				...
				$result = '<br /><table class="tb tb2 nobdb" width="100%" border="0" cellpadding="0" cellspacing="0"><tr class="partition"><td>'.$lang['tpl_diff_current'].'</td><td>&nbsp;</td><td>'.$lang['tpl_diff_default'].'</td></tr>';
				...
				$result .= '</table><br /><table class="tb tb2 nobdb" width="100%" border="0" cellpadding="4" cellspacing="0">'.
					'<tr><th class="diff-deleted" style="text-align: center">'.$lang['tpl_diff_deleted'].'</th><th class="diff-notext">&nbsp;</th></tr>'.
					'<tr><th class="diff-changed" style="text-align: center" colspan="2">'.$lang['tpl_diff_changed'].'&nbsp;</th></tr>'.
					'<tr><th class="diff-notext">&nbsp;</th><th class="diff-added" style="text-align: center">'.$lang['tpl_diff_added'].'</th></tr></table>';
				$checkresult = $result;
			}
		} else {
			$onclickevent = 'onclick="return confirm(\''.$lang['tpl_edit_default_overwriteconfirm'].'\')"';
		}
...
function HighlightAll(obj) {
	...
	if(document.all) {
		...
		window.status = "<?=$lang['tpl_edit_clickboard']?>";
...
function findInPage(obj, str, noalert) {
	...
	if(document.layers) {
		...
		if(n == 0 && !noalert) {
			alert("<?=$lang['tpl_edit_keyword_not_found']?>");
		}
	}
	if(document.all) {
...
<?
		shownav('forum', 'tpl_edit');
		showsubmenu("$lang[tpl_edit] - $template[name] $fn - $lang[lastmodified]: $filemtime");
...
<tr><td><b><?=$lang['tpl_variable']?></b></td><td><b><?=$lang['tpl_value']?></b></td></tr>
<?=$langinputs?>
</table>
</div>
<?
...
?>
<input type="button" class="btn" value="<?=$lang['return']?>" accesskey="e" onClick="location.href='admincp.php?action=templates&edit=<?=$templateid?>&keyword=<?=$keywordenc?>'">
...
<?
		if($allowedittpls) {
			...
			if($directorys) {
				echo $lang['tpl_copyto_otherdirs']."<select id=\"copyto\">".
					"$directorys</select> <input style=\"vertical-align: middle\" type=\"button\" class=\"btn\" value=\"$lang[tpl_start_copy]\" ".
					"accesskey=\"r\" onclick=\"if(\$('copyto').value == 1 && confirm('$lang[tpl_edit_default_overwriteconfirm]') || \$('copyto').value != 1) location.href='admincp.php?action=templates&operation=copy&templateid={$template['templateid']}&fn={$fn}&copyto='+\$('copyto').value\">";
			}
			echo $resetbutton;
		}
		...
		checkpermission('tpledit');
		if(!$confirmed) {
			cpmsg('tpl_delete_confirm', "admincp.php?action=templates&operation=edit&templateid=$templateid&fn=$fn&delete=yes", 'form');
		} else {
			if(@unlink($filename)) {
				cpmsg('tpl_delete_succeed', "admincp.php?action=templates&edit=$templateid", 'succeed');
			} else {
		...
		checkpermission('tpledit');
		if(!$confirmed) {
			cpmsg('tpl_reset_confirm', "admincp.php?action=templates&operation=edit&templateid=$templateid&fn=$fn&keyword=$keywordenc&reset=yes", 'form');
		} else {
			...
			cpmsg('tpl_reset_succeed', "admincp.php?action=templates&edit=$templateid&keyword=$keywordenc", 'succeed');
		}
		...
		cpmsg('tpl_edit_succeed', "admincp.php?action=templates&edit=$templateid&keyword=$keywordenc", 'succeed');
	...
	@fclose($fp);
	cpmsg('tpl_add_succeed', "admincp.php?action=templates&operation=edit&templateid=1&fn=$name.htm", 'succeed');
