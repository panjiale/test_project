...
/*
	...
	$Id: moderate.inc.php 13486 2008-04-18 04:32:01Z liuqiang $
*/
...
if($operation == 'members') {
	...
	if($do == 'mod') {
		...
		if(!submitcheck('modsubmit')) {
			...
			$multipage = multi($db->result($query, 0), $memberperpage, $page, 'admincp.php?action=moderate&operation=members&sendemail=$sendemail');
			...
			cpmsg('moderate_members_succeed', "admincp.php?action=moderate&operation=members&page=$page", 'succeed');
		...
		if(!submitcheck('prunesubmit', 1)) {
			...
			if(!$membernum = $db->num_rows($query)) {
				...
				cpmsg('members_delete_confirm', "admincp.php?action=moderate&operation=members&do=del&submitmore=".rawurlencode($submitmore)."&regbefore=".rawurlencode($regbefore)."&regip=".rawurlencode($regip)."&prunesubmit=yes", 'form');
			} else {
				$uids = 0;
				while($member = $db->fetch_array($query)) {
					$uids .= ','.$member['uid'];
...
if($operation == 'threads') {
	...
	if(!submitcheck('modsubmit')) {
		...
		$multipage = multi($modcount, $tpp, $page, "admincp.php?action=moderate&operation=threads&filter=$filter&modfid=$modfid");
		...
		showtableheader("$lang[select]: <select style=\"margin: 0px;\" onchange=\"if(this.options[this.selectedIndex].value != '') {window.location=('admincp.php?action=moderate&operation=threads&modfid=$modfid&filter='+this.options[this.selectedIndex].value+'&amp;sid=5ScwCd');
		}\">$filteroptions</select> <select style=\"margin: 0px;\" onchange=\"if(this.options[this.selectedIndex].value != '') {window.location=('admincp.php?action=moderate&operation=threads&filter=$filter&modfid='+this.options[this.selectedIndex].value+'&amp;sid=5ScwCd');
		}\">$forumoptions</select>");
		...
		while($thread = $db->fetch_array($query)) {
			...
			showtablerow("id=\"mod_$thread[tid]_row1\"", array('rowspan="3" class="rowform threadopt" style="width:80px;"', 'class="threadtitle"'), array(
				...
				"<h3><a href=\"forumdisplay.php?fid=$thread[fid]\" target=\"_blank\">$thread[forumname]</a> &raquo; <a href=\"viewthread.php?tid=$thread[tid]\" target=\"_blank\">$thread[subject]</a></h3><p><span class=\"bold\">$lang[author]:</span> $thread[author] ($thread[useip]) &nbsp;&nbsp; <span class=\"bold\">$lang[time]:</span> $thread[dateline]</p>"
			));
			...
			showtablerow("id=\"mod_$thread[tid]_row3\"", 'class="threadopt threadtitle" colspan="2"', "<a href=\"post.php?action=edit&fid=$thread[fid]&tid=$thread[tid]&pid=$thread[pid]&page=1&mod=edit\" target=\"_blank\">".$lang['moderate_edit_thread']."</a> &nbsp;&nbsp;|&nbsp;&nbsp; ".$lang['moderate_reasonpm']."&nbsp; <input type=\"text\" class=\"txt\" name=\"pm_$thread[tid]\" id=\"pm_$thread[tid]\" style=\"margin: 0px;\"> &nbsp; <select style=\"margin: 0px;\" onchange=\"$('pm_$thread[tid]').value=this.value\">$modreasonoptions</select>");
		}
		...
		if($pmlist) {
			foreach($pmlist as $pm) {
				...
				$tid = intval($pm['tid']);
				sendpm($pm['authorid'], $pm['action'].'subject', $pm['action'].'message');
			}
		}
		...
		if($validates) {
			...
			cpmsg('moderate_threads_succeed', 'admincp.php?action=moderate&operation=threads', 'succeed');
		}
	...
	if(!submitcheck('modsubmit')) {
		...
		$multipage = multi($modcount, $ppp, $page, "admincp.php?action=moderate&operation=replies&filter=$filter&modfid=$modfid");

		shownav('topic', $lang['moderate_posts']);
		showsubmenu('nav_moderate_posts', array(
			array('nav_moderate_threads', 'moderate&operation=threads', 0),
		...
		showtableheader("$lang[select]: <select style=\"margin: 0px;\" onchange=\"if(this.options[this.selectedIndex].value != '') {window.location=('admincp.php?action=moderate&operation=replies&modfid=$modfid&filter='+this.options[this.selectedIndex].value+'&amp;sid=5ScwCd');}\">$filteroptions</select> <select style=\"margin: 0px;\" onchange=\"if(this.options[this.selectedIndex].value != '') {window.location=('admincp.php?action=moderate&operation=replies&filter=$filter&modfid='+this.options[this.selectedIndex].value+'&amp;sid=5ScwCd');}\">$forumoptions</select>");
		...
		while($post = $db->fetch_array($query)) {
			...
			showtablerow("id=\"mod_$post[pid]_row1\"", array('rowspan="3" class="rowform threadopt" style="width:80px;"', 'class="threadtitle"'), array(
				...
				"<h3><a href=\"forumdisplay.php?fid=$post[fid]\" target=\"_blank\">$post[forumname]</a> &raquo; <a href=\"viewthread.php?tid=$post[tid]\" target=\"_blank\">$post[tsubject]</a> &raquo; <b>$post[subject]</b></h3><p><span class=\"bold\">$lang[author]:</span> $post[author] ($post[useip]) &nbsp;&nbsp; <span class=\"bold\">$lang[time]:</span> $post[dateline]</p>"
			));
			...
			showtablerow("id=\"mod_$post[pid]_row3\"", 'class="threadopt threadtitle" colspan="2"', "<a href=\"post.php?action=edit&fid=$post[fid]&tid=$post[pid]&pid=$post[pid]&page=1&mod=edit\" target=\"_blank\">".$lang['moderate_edit_post']."</a> &nbsp;&nbsp;|&nbsp;&nbsp; ".$lang['moderate_reasonpm']."&nbsp; <input type=\"text\" class=\"txt\" name=\"pm_$post[pid]\" id=\"pm_$post[pid]\" style=\"margin: 0px;\"> &nbsp; <select style=\"margin: 0px;\" onchange=\"$('pm_$post[pid]').value=this.value\">$modreasonoptions</select>");
		...
		cpmsg('moderate_replies_succeed', "admincp.php?action=moderate&operation=replies&page=$page&filter=$filter&modfid=$modfid", 'succeed');
