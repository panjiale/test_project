...
/*
	...
	$Id: advertisements.inc.php 13487 2008-04-18 05:00:01Z liuqiang $
*/
...
if(empty($operation)) {
	...
	if(!submitcheck('advsubmit')) {
		...
		shownav('adv', 'nav_adv');
		showsubmenu('nav_adv', array(
			array('config', 'advertisements&operation=config', 0),
			array('admin', 'advertisements', 1),
			array(array('menu' => 'add', 'submenu' => array(
				array('adv_type_headerbanner', 'advertisements&operation=advadd&type=headerbanner'),
				array('adv_type_footerbanner', 'advertisements&operation=advadd&type=footerbanner'),
				array('adv_type_text', 'advertisements&operation=advadd&type=text'),
				array('adv_type_thread', 'advertisements&operation=advadd&type=thread'),
				array('adv_type_interthread', 'advertisements&operation=advadd&type=interthread'),
				array('adv_type_float', 'advertisements&operation=advadd&type=float'),
				array('adv_type_couplebanner', 'advertisements&operation=advadd&type=couplebanner'),
				array('adv_type_intercat', 'advertisements&operation=advadd&type=intercat'),

			)), '', 0)
		));
		showformheader('advertisements');
		showtableheader();
		...
		while($adv = $db->fetch_array($query)) {
			...
			showtablerow('', array('class="td25"', 'class="td28"', 'class="td25"'), array(
				...
				$adv[targets],
				"<a href=\"admincp.php?action=advertisements&operation=advedit&advid=$adv[advid]\" class=\"act\">$lang[detail]</a>"
			));
		}

		$multipage = multi($advnum, 15, $page, 'admincp.php?action=advertisements'.($title ? "&title=$title" : '').($type ? "&type=$type" : '').($starttime ? "&starttime=$starttime" : '').($orderby ? "&orderby=$orderby" : ''), 0, 3, TRUE, TRUE);
...
<input type="submit" class="btn" name="searchsubmit" value="'.lang('search').'" onclick="if(this.form.title.value==\''.lang('adv_inputtitle').'\'){this.form.title.value=\'\'}window.loacation=\'admincp.php?action=advertisements&title=\'+this.form.title.value+\'&starttime=\'+this.form.starttime.value+\'&type=\'+this.form.type.value+\'&orderby=\'+this.form.orderby.value;"> &nbsp;
		');
		showtablefooter();
		showformfooter();
		...
		cpmsg('adv_update_succeed', 'admincp.php?action=advertisements', 'succeed');
	...
	if(!submitcheck('advsubmit')) {
		...
		showformheader("advertisements&operation=$operation".($operation == 'advadd' ? '&type='.$type : '&advid='.$advid));
		...
		cpmsg('adv_succeed', 'admincp.php?action=advertisements', 'succeed');
	...
	if(!submitcheck('configsubmit')) {
		...
		shownav('adv', 'nav_adv');
		showsubmenu('nav_adv', array(
			array('config', 'advertisements&operation=config', 1),
			array('admin', 'advertisements', 0),
			array(array('menu' => 'add', 'submenu' => array(
				array('adv_type_headerbanner', 'advertisements&operation=advadd&type=headerbanner'),
				array('adv_type_footerbanner', 'advertisements&operation=advadd&type=footerbanner'),
		...
		showformheader('advertisements&operation=config');
		showtableheader();
		showsetting('settings_adv_mode', array('admodenew', array(array(0, lang('settings_adv_mode_0')), array(1, lang('settings_adv_mode_1')))), $admode, 'select');
		...
		cpmsg('settings_adv_update_succeed', 'admincp.php?action=advertisements&operation=config', 'succeed');
