...
/*
	...
	$Id: tools.inc.php 13369 2008-04-11 07:08:25Z liuqiang $
*/
...
if($operation == 'updatecache') {
	...
	if($step == 1) {
		cpmsg('<input type=\"checkbox\" name=\"type[]\" value=\"data\" id=\"datacache\" class=\"checkbox\" /><label for=\"datacache\">'.$lang[updatecache_data].'</label><input type=\"checkbox\" name=\"type[]\" value=\"tpl\" id=\"tplcache\" class=\"checkbox\" /><label for=\"tplcache\">'.$lang[updatecache_tpl].'</label>', 'admincp.php?action=tools&operation=updatecache&step=2', 'form', '', FALSE);
	} elseif($step == 2) {
		$type = implode('_', (array)$type);
		cpmsg(lang('updatecache_waiting'), "admincp.php?action=tools&operation=updatecache&step=3&type=$type", 'loading', '', FALSE);
	} elseif($step == 3) {
		$type = explode('_', $type);
	...
	if(!submitcheck('submit')) {
		...
		shownav('tools', 'jswizard_tag');
		showsubmenu('jswizard_tag');
		showformheader('tools&operation=relatedtag');
		
		if(!$tagstatus) {
			showtips('settings_relatetag_tips');
		}
		
		showtableheader();
		showsetting('settings_relatetag', 'relatedtagstatusnew', $settings['relatedtagstatus'], 'radio');
		...
		foreach($applist as $data) {
			...
			showtitle($data['name']);
			showsetting('jswizard_tag_name', 'relatedtagnew[name]['.$appid.']', $name, 'text');
			showsetting('jswizard_tag_status', 'relatedtagnew[status]['.$appid.']', $status[0] ? 0 : 1, 'radio');
			showsetting('jswizard_tag_order', 'relatedtagnew[order]['.$appid.']', $relatedtag['order'][$appid], 'text');
			showsetting('jswizard_tag_items', 'relatedtagnew[limit]['.$appid.']', intval($relatedtag['limit'][$appid]), 'text');
			echo '<tr class="nobg"><td class="td27" colspan="2">'.$lang['jswizard_tag_tpl'].'</td></tr><tr><td class="vtop rowform">'.
			...
			if(is_array($data['tagtemplates']['fields'])) {
				foreach($data['tagtemplates']['fields'] as $field => $memo) {
					echo '<a onclick="insertunit(\'{'.$field.'}\', \'jstemplate_'.$appid.'\')" href="###">{'.$field.'}</a> '.$lang['jswizard_tag_memo'].' '.$memo.'<br />';
				}
			}
			echo '</td></tr>';
		...
		cpmsg('jswizard_relatedtag_succeed', 'admincp.php?action=tools&operation=relatedtag', 'succeed');
	...
	if($step == 1) {
		cpmsg(lang('fileperms_check_note'), 'admincp.php?action=tools&operation=fileperms&step=2', 'button', '', FALSE);
	} elseif($step == 2) {
		cpmsg(lang('fileperms_check_waiting'), 'admincp.php?action=tools&operation=fileperms&step=3', 'loading', '', FALSE);
	} elseif($step == 3) {
		...
		echo '<div class="colorbox nobg"><ul class="fileperms">'.$result.'</ul></div>';
	}
}
