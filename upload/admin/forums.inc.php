...
/*
	...
	$Id: forums.inc.php 13444 2008-04-16 03:12:04Z liuqiang $
*/
...
<?
	...
	if(!submitcheck('detailsubmit') && !submitcheck('saveconfigsubmit')) {
		...
		if($forum['type'] == 'group') {
			...
			showsetting('forums_edit_autoclose_time', 'autoclosetimenew', $forum['autoclose'], 'text');
			showtagfooter('tbody');
...
<?php
	...
	if($ajax) {
		...
		if(!$confirmed) {
			...
			cpmsg('forums_delete_alarm', 'admincp.php?action=forums&operation=delete&fid=2', 'loadingform', '<div id="percent">0%</div>', FALSE);
