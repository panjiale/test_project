...
/*
	...
	$Id: magics.inc.php 13369 2008-04-11 07:08:25Z liuqiang $
*/
...
if($operation == 'config') {
	...
	if(!submitcheck('magicsubmit')) {
		...
		showtablerow('class="nobg"', 'colspan="2"', '<input type="submit" class="btn" name="magicsubmit" value="'.$lang['submit'].'"  />');
		showtablefooter();
		showformfooter();
		...
		cpmsg('magics_config_succeed', 'admincp.php?action=magics&operation=config', 'succeed');
	}
	...
	if(!submitcheck('magicsubmit')) {
		...
		shownav('misc', 'magics', 'admin');
		showsubmenu('nav_magics', array(
			array('config', 'magics&operation=config', 0),
			array('admin', 'magics&operation=magic', 1),
			array('nav_magics_market', 'magics&operation=market', 0)
		));
...
<?
		showformheader('magics&operation=magic');
		...
		while($magic = $db->fetch_array($query)) {
			...
			showtablerow('', array('class="td25"', 'class="td28"', 'class="td25"', 'class="td28"', '', 'class="td28"', 'class="td28"', 'class="td23"', 'class="td25"', 'class="td25"'), array(
				...
				"<a href=\"admincp.php?action=magics&operation=magic&typeid=$magic[type]\">$magictype</a>",
				"<input type=\"hidden\" name=\"identifier[$magic[magicid]]\" value=\"$magic[identifier]\">$magic[identifier]",
				"<a href=\"admincp.php?action=magics&operation=edit&magicid=$magic[magicid]\" class=\"act\">$lang[detail]</a>"
			));
		}
		...
		updatecache('magics');
		cpmsg('magics_data_succeed', 'admincp.php?action=magics&operation=magic', 'succeed');
	...
	if(!submitcheck('magiceditsubmit')) {
		...
		showtablerow('class="nobg"', 'class="td27"', $lang['magics_edit_usergroupperm'].':<input class="checkbox" type="checkbox" name="chkall1" onclick="checkAll(\'prefix\', this.form, \'usergroupsperm\', \'chkall1\', true)" id="chkall1" /><label for="chkall1"> '.lang('select_all').'</label>');
		showtablerow('', 'colspan="2"', mcheckbox('usergroupsperm', $groups, explode("\t", $magicperm['usergroups'])));

		if($magic['type'] == 2 || $magic['type'] == 3) {
			showtablerow('class="nobg"', 'class="td27"', $lang['magics_edit_targetgroupperm'].':<input class="checkbox" type="checkbox" name="chkall2" onclick="checkAll(\'prefix\', this.form, \'targetgroupsperm\', \'chkall2\', true)" id="chkall2" /><label for="chkall2"> '.lang('select_all').'</label>');
			showtablerow('', 'colspan="2"', mcheckbox('targetgroupsperm', $groups, explode("\t", $magicperm['targetgroups'])));
		}
		if($magic['type'] == 1) {
			showtablerow('class="nobg"', 'class="td27"', $lang['magics_edit_forumperm'].':<input class="checkbox" type="checkbox" name="chkall3" onclick="checkAll(\'prefix\', this.form, \'forumperm\', \'chkall3\', true)" id="chkall3" /><label for="chkall3"> '.lang('select_all').'</label>');
	...
	if(!submitcheck('marketsubmit')) {
		...
		shownav('misc', 'magics', 'nav_magics_market');
		showsubmenu('nav_magics', array(
			array('config', 'magics&operation=config', 0),
			array('admin', 'magics&operation=magic', 0),
		...
		cpmsg('magics_data_succeed', 'admincp.php?action=magics&operation=market', 'succeed');
