...
/*
	...
	$Id: qihoo.inc.php 13292 2008-04-07 16:38:28Z liuqiang $
*/
...
if($operation == 'config') {
	...
	if(!submitcheck('qihoosubmit')) {
		...
		cpmsg('qihoo_succeed', 'admincp.php?action=qihoo&operation=config', 'succeed');
	...
	if(!submitcheck('qihoosubmit')) {
		...
		showsetting('qihoo_relatedthreads_type', '', '', '<ul class="nofloat" onmouseover="altStyle(this);">'.
			'<li'.($checktype['blog'] ? ' class="checked"' : '').'><input class="checkbox" type="checkbox" name="settingsnew[type][blog]" value="blog" '.$checktype['blog'].'> '.$lang['qihoo_relatedthreads_type_blog'].'</li>'.
	...
	if(!submitcheck('topicsubmit')) {
		...
		$db->query("UPDATE {$tablepre}settings SET value='".addslashes(serialize($qihoo))."' WHERE variable='qihoo'");
		updatecache('settings');
		cpmsg('qihoo_topics_succeed', 'admincp.php?action=qihoo&operation=topics', 'succeed');
