...
/*
	...
	$Id: database.inc.php 13486 2008-04-18 04:32:01Z liuqiang $
*/
...
if($operation == 'export') {
	...
	if(!submitcheck('exportsubmit', 1)) {
		...
		showsubmenu('nav_db', array(
			array('nav_db_export', 'database&operation=export', 1),
		...
		showformheader('database&operation=export&setup=1');
		showtableheader();
		showsetting('database_export_type', array('type', array(
			array('discuz', $lang['database_export_discuz'], array('showtables' => 'none')),
			array('custom', $lang['database_export_custom'], array('showtables' => ''))
		)), 'discuz', 'mradio');


		showtagheader('tbody', 'showtables');
		showtablerow('', '', '<input class="checkbox" name="chkall" onclick="checkAll(\'prefix\', this.form, \'customtables\', \'chkall\', true)" checked="checked" type="checkbox" id="chkalltables" /><label for="chkalltables"> '.lang('database_export_custom_select_all').' - '.lang('database_export_discuz_table')).'</label>';
		showtablerow('', 'colspan="2"', mcheckbox('customtables', $dztables));
		showtagfooter('tbody');

		showtagheader('tbody', 'advanceoption');
		showsetting('database_export_method', '', '', '<ul class="nofloat"><li><input class="radio" type="radio" name="method" value="shell" '.$shelldisabled.' onclick="if(\''.intval($db->version() < '4.1').'\') {if(this.form.sqlcompat[2].checked==true) this.form.sqlcompat[0].checked=true; this.form.sqlcompat[2].disabled=true; this.form.sizelimit.disabled=true;} else {this.form.sqlcharset[0].checked=true; for(var i=1; i<=5; i++) {if(this.form.sqlcharset[i]) this.form.sqlcharset[i].disabled=true;}}" id="method_shell" /><label="method_shell"> '.$lang['database_export_shell'].'</label></li><li><input class="radio" type="radio" name="method" value="multivol" checked="checked" onclick="this.form.sqlcompat[2].disabled=false; this.form.sizelimit.disabled=false; for(var i=1; i<=5; i++) {if(this.form.sqlcharset[i]) this.form.sqlcharset[i].disabled=false;}" id="method_multivol" /><label for="method_multivol"> '.$lang['database_export_multivol'].'</label> <input type="text" class="txt" size="40" name="sizelimit" value="2048" /></li></ul>');
		showtitle('database_export_options');
		showsetting('database_export_options_extended_insert', 'extendins', 0, 'radio');
		showsetting('database_export_options_sql_compatible', array('sqlcompat', array(
			array('', $lang['default']),
			array('MYSQL40', 'MySQL 3.23/4.0.x'),
			array('MYSQL41', 'MySQL 4.1.x/5.x')
		)), '', 'mradio');
		showsetting('database_export_options_charset', array('sqlcharset', array(
			array('', lang('default')),
			$dbcharset ? array($dbcharset, strtoupper($dbcharset)) : array(),
			$db->version() > '4.1' && $dbcharset != 'utf8' ? array('utf8', 'UTF-8') : array()
		), TRUE), 0, 'mradio');
		showsetting('database_export_usehex', 'usehex', 1, 'radio');
		if(function_exists('gzcompress')) {
			showsetting('database_export_usezip', array('usezip', array(
				array('1', $lang['database_export_zip_1']),
				array('2', $lang['database_export_zip_2']),
				array('0', $lang['database_export_zip_3'])
			)), 0, 'mradio');
		}
		showsetting('database_export_filename', '', '', '<input type="text" class="txt" name="filename" value="'.$defaultfilename.'" />.sql');
		showtagfooter('tbody');
	...
	showsubmenu('nav_db', array(
		...
		array('nav_db_import', 'database&operation=import', 0),
		array('nav_db_runquery', 'database&operation=runquery', 0),
		array('nav_db_optimize', 'database&operation=optimize', 1),
		array('nav_db_dbcheck', 'database&operation=dbcheck', 0)

	));
	showtips('database_optimize_tips');
	showformheader('database&operation=optimize');
	showtableheader('database_optimize_tables');
	showsubtitle(array('', 'database_optimize_table_name', 'type', 'database_optimize_rows', 'database_optimize_data', 'database_optimize_index', 'database_optimize_frag'));
	...
	if(!submitcheck('optimizesubmit')) {
		...
		if(empty($totalsize)) {
			showtablerow('', 'colspan="6"', $lang['database_optimize_done']);
		} else {
			showtablerow('', 'colspan="6"', $lang['database_optimize_used'].' '.sizecount($totalsize));
			showsubmit('optimizesubmit', 'submit', '<input name="chkall" id="chkall" class="checkbox" onclick="checkAll(\'prefix\', this.form)" checked="checked" type="checkbox" /><label for="chkall">'.$lang[database_optimize_opt].'</label>');
		}
	...
	if($step == 3) {
		...
		if(submitcheck('repairsubmit') && (!empty($repair) || !empty($setting) || !empty($repairtable) || !empty($missingtable))) {
			...
			if($errorcount) {
				...
				cpmsg('dbcheck_repair_completed', 'admincp.php?action=database&operation=dbcheck&step=3', 'succeed');
			}
		}
			...
			if(($modifylist || $dellist) && !in_array($dbtable, $excepttables)) {
				...
				foreach($modifylist as $value) {
					$showlist .= "<tr><td><input name=\"repair[]\" class=\"checkbox\" type=\"checkbox\" value=\"$dbtable|$value[Field]|modify\"> <b>".$value['Field']."</b> ".
						...
						' '.$value['Extra']."</td><td>".
						"<em class=\"edited\">$lang[dbcheck_modify]</em></td></tr>";
				}
	...
	showsubmenu('nav_db', array(
		...
		array('nav_db_runquery', 'database&operation=runquery', 0),
		array('nav_db_optimize', 'database&operation=optimize', 0),
		array('nav_db_dbcheck', 'database&operation=dbcheck', 1)
	...
	if($step == 1) {
		cpmsg($lang[dbcheck_tips_step1], 'admincp.php?action=database&operation=dbcheck&step=2', 'button', '', FALSE);
	} elseif($step == 2) {
		cpmsg(lang('dbcheck_verifying'), "admincp.php?action=database&operation=dbcheck&step=3", 'loading', '', FALSE);
	} elseif($step == 3) {
...
EOT;
		showformheader('database&operation=dbcheck&step=3', 'fixpadding');
		showtableheader();
		echo $showlist;
		showtablefooter();
		showformfooter();
