...
$Id: members.inc.php 13242 2008-04-02 05:47:44Z cnteacher $
*/
...
if($op == 'edit') {
	...
	if($member && submitcheck('editsubmit')) {
	...
	$member = loadmember($uid, $username, $error);
	if($member && submitcheck('bansubmit')) {
		$sql = 'uid=uid';
		$reason = trim($reason);
		if(!$reason && ($reasonpm == 1 || $reasonpm == 3)) {
			acpmsg('members_edit_reason_invalid');
		}
