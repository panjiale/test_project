...
$Id: report.inc.php 12294 2008-01-24 07:58:55Z cnteacher $
*/
...
function delete_access($uid, $fid) {
	global $db, $tablepre;
	$db->query("DELETE FROM {$tablepre}access WHERE uid='$uid' AND fid='$fid'", 'UNBUFFERED');
	$mask = $db->result_first("SELECT count(*) FROM {$tablepre}access WHERE uid='$uid'");
	if(!$mask) {
		$db->query("UPDATE {$tablepre}members SET accessmasks=''", 'UNBUFFERED');
	}
}
