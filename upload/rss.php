...
/*
	...
	$Id: rss.php 12155 2008-01-15 09:04:11Z monkey $
*/
	...
	foreach($_DCACHE['forums'] as $fid => $forum) {
		if($forum['type'] != 'group') {
			...
			while($thread = $db->fetch_array($query)) {
				...
				$thread['description'] = $thread['readperm'] > 0 || $thread['price'] > 0 || $thread['status'] & 1 ? '' : addslashes(cutstr(dhtmlspecialchars(preg_replace("/(\[.+\])/s", '', strip_tags(nl2br($thread['message'])))), 255));
