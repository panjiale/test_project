...
/*
	...
	$Id: admincp.php 13358 2008-04-10 13:48:51Z liuqiang $
*/
...
if($cpaccess == 0 || (!$discuz_secques && $admincp['forcesecques'])) {
	...
	if($admin_password != '') {
		...
		$ucresult = uc_user_login($discuz_uid, $admin_password, 1);
		if($ucresult[0] > 0) {
	...
	if(empty($action) || isset($frames)) {
		...
		if($radminid != $groupid) {
			...
			if(in_array($action.($operation ? '_'.$operation : ''), $dactionarray)) {
				cpheader();
