...
/*
	...
	$Id: newreply.inc.php 13268 2008-04-03 05:55:50Z monkey $
*/
...
if(!submitcheck('replysubmit', 0, $seccodecheck, $secqaacheck)) {
	...
	if($modnewreplies) {
		...
		$feed = array();
		if($addfeed && $thread['authorid'] != $discuz_uid) {
			if($special == 2 && !empty($trade) && !empty($item_name) && !empty($item_price)) {
				...
				$feed['title_data'] = array(
					'subject' => "<a href=\"{$boardurl}redirect.php?goto=findpost&pid=$pid&ptid=$tid\">$thread[subject]</a>",
					'author' => "<a href=\"space.php?uid=$thread[authorid]\">$thread[author]</a>"
				);
			}
			postfeed($feed);
