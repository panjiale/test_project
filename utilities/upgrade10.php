...
$msglang = array(
	...
	'uc_ip_invalid' => '<font color="red">无法解析该域名，请填写站点的IP。</font>',
	'uc_admin_invalid' => '登录 UCenter 的管理员帐号密码错误，请返回检查。',
...
<?
	...
	if(empty($_POST['ucsubmit']) && !getgpc('start')) {
		...
		if(file_exists(DISCUZ_ROOT.'forumdata/upgrademaxuid.log')) {
			...
			$maxuid = $maxuid[0];
			instmsg('会员数据导入完毕。', '?step='.($maxuid > 0 ? 'merge' : '5').'&urladd='.$urladd.'&maxuid='.$maxuid, $uchidden);
		}
...
<table width="80%" cellspacing="1" bgcolor="#000000" border="0" align="center">
<tr bgcolor="#3A4273"><td style="color: #FFFFFF; padding-left: 10px" colspan="2">会员 ID 起始值</td></tr>
<tr>
<td class="altbg1">会员 ID 起始值:</td>
<td class="altbg2"><input class="txt" type="text" name="maxuidset" value="<?=$maxuid?>" size="10">
起始会员 ID 必须大于等于 <?=$maxuid?>。如填写 10000 那么，原 ID 为 888 的会员将变为 10888。</td>
</tr>
</table>
<center>
<input type="button" name="ucsubmit" value=" <?=$lang['old_step']?> " style="height: 25" onclick="history.back()">&nbsp;
...
<?
	...
	if(!$total) {
		...
		if($maxuidset < $maxuid) {
			@unlink(DISCUZ_ROOT.'forumdata/upgrademaxuid.log');
			instmsg('起始会员 ID 必须大于等于 '.$maxuid.'，请返回重新填写。', '?step=4&urladd='.$urladd, $uchidden);
		} else {
			$maxuid = $maxuidset;
		}
	...
	$end = $start + $limit;
	instmsg("会员数据导入到 UCenter $start / $total ...", '?step=4&'.$urladd.'&start='.$end.'&total='.$total.'&maxuid='.$maxuid.'&lastuid='.$lastuid, $uchidden);
	...
	if($total == 0 || $total <= $start) {
		instmsg('会员数据合并完毕。', '?step=5&urladd='.$urladd, $uchidden);
	}
	...
	if(!in_array($table, $logs)) {
		...
		if($stepfield) {
			...
			if($mtotal != 0 && $mtotal > $mstart) {
				...
				instmsg("正在处理会员合并数据 {$tablepre}$table $mstart / $mtotal ...", '?step=merge&'.$urladd.'&start='.$start.'&maxuid='.$maxuid, $uchidden);
			} else {
	...
	instmsg("{$tablepre}$table 表合并完毕。", '?step=merge&'.$urladd.'&start='.$end.'&maxuid='.$maxuid, $uchidden);
